        ...
        with savReaderWriter.SavReader(params[0]) as reader:
            ...
            for line in reader:
                ...
                for i in range(len(searchIndexes1)):
                    query_insert += (str(line[searchIndexes1[i]]) if (searchIndexes1[i] is not None and line[searchIndexes1[i]] is not None) else "Null")
                ...
                for i in range(len(searchIndexes2)):
                    query_insert += (str(line[searchIndexes2[i]]) if (searchIndexes2[i] is not None and line[searchIndexes2[i]] is not None) else "Null")
